...
class Test_Real_Time_Learning_Algorithm(unittest.TestCase):

    @classmethod
    def setUpClass(cls):

        cls.task = Add_Task(4, 6, deterministic=True, tau_task=4)
        cls.data = cls.task.gen_data(50, 0)

        n_in = cls.task.n_in
        n_h = 2
        n_out = cls.task.n_out

        cls.W_in = np.eye(n_in)
        cls.W_rec = np.eye(n_h)
        cls.W_out = np.eye(n_out)
        cls.W_FB = -np.ones((n_out, n_in)) + np.eye(n_out)
    ...
    def test_get_outer_grads(self):
        ...
        self.rnn = RNN(self.W_in, self.W_rec, self.W_out,
        ...
        self.learn_alg = Real_Time_Learning_Algorithm(self.rnn)

        self.rnn.a = np.ones(2)
        self.rnn.error = np.ones(2) * 0.5
    ...
    def test_propagate_feedback_to_hidden(self):
        ...
        self.learn_alg = Real_Time_Learning_Algorithm(self.rnn, W_FB=self.W_FB)
        self.learn_alg.propagate_feedback_to_hidden()
        correct_q = -np.ones(2) * 0.5
        self.assertTrue(np.isclose(self.learn_alg.q,
                                   correct_q).all())
    ...
    def test_L2_regularization(self):
        ...
        self.learn_alg = Real_Time_Learning_Algorithm(self.rnn, L2_reg=0.1)
...
class Test_Only_Output_Weights(unittest.TestCase):
